...
<?php
	...
	if (file_exists("fx_config.php")) {
		...
		<p><a href="?reinstall">REINSTALL</a></p>';
		die();
	}
	...
	if (isset($_POST['flexidb_install'])) {
		...
		$db_driver = 'mysql';//$_POST['db_driver'];
		...
		if (!$errors) {
			...
			if (!file_exists('fx_config.php')) {
				...
				if (!$fx_db) {
					...
					$version_info = dirname(__FILE__).'\version_info.txt';

					if ($version_info = file_get_contents($version_info)) {
						...
						add_log_message('flexidb_install', 'Unable to read '.$version_info);
					}
