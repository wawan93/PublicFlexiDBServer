...
abstract class FX_API
{
	...
	public function __construct($request)
	{
		...
		if ($data && $this -> args === NULL) {
			//add_log_message($this -> endpoint.'.'.$this -> verb, print_r($this -> args, true));
		...
		elseif (isset($this -> args['object_type_1_id']) && !is_system_type($this -> args['object_type_1_id'])) {
			$object_type_id = $this -> args['object_type_1_id'];
		}
		elseif (isset($this -> args['object_type_2_id']) && !is_system_type($this -> args['object_type_2_id'])) {
			$object_type_id = $this -> args['object_type_2_id'];
		}		
		
		//$object_type_id = isset($this -> args['object_type_id']) ? $this -> args['object_type_id'] : 0;
		$type_data = get_type($object_type_id, 'none');
		
		if (!is_fx_error($type_data) && $type_data) {
			$this -> schema_id = $type_data['schema_id'];
		}

		$this -> permission = isset($user_instance['schema_permissions'][$this->schema_id][$object_type_id]) ? $user_instance['schema_permissions'][$this->schema_id][$object_type_id] : 0;
    ...
    private function _clean_inputs($data)
	{
		...
		//Check and decode if input data has json format
