...
class Task_Send_Sms extends FX_Task
{
	function action($args, $result)
	{
		...
		if(!$from) return new FX_Error(__CLASS__, 'Unknow sender number.');
		if(!$to) return new FX_Error(__CLASS__, 'Unknow sender number.');
		...
		try
		{
			if(strlen($message) > 160) {
				$messages = str_split($message, 154);
				for($i = 0; $i < count($messages); $i++) {
					$msg = ($i ? '...' : '').$messages[$i].($i != count($messages)-1 ? '...' : '');
					$result = $client -> account -> sms_messages -> create($from, $to, $msg);
				}
			}
			else {
				$result = $client -> account -> sms_messages -> create($from, $to, $message);
			}
