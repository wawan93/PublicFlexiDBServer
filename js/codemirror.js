...
window.CodeMirror = (function() {
  ...
  function buildLineElement(cm, line, lineNo, dims) {
    ...
    if (!cm.options.lineNumbers && !markers && !line.bgClass && !line.wrapClass &&
        (!line.elements || !line.elements.length)) return lineElement;
    ...
    if (line.elements)
      for (var i = 0, ws = line.elements; i < ws.length; ++i) {
  ...
  function intoCoordSystem(cm, lineObj, rect, context) {
    if (lineObj.elements) for (var i = 0; i < lineObj.elements.length; ++i) if (lineObj.elements[i].above) {
      var size = lineObj.elements[i].node.offsetHeight;
  ...
  CodeMirror.prototype = {
    ...
    addLineWidget: operation(null, function(handle, node, options) {
      ...
      changeLine(this, handle, function(line) {
        (line.elements || (line.elements = [])).push(widget);
        widget.line = line;
        return true;
      });
      return widget;
    ...
    removeLineWidget: operation(null, function(widget) {
      var ws = widget.line.elements, no = lineNo(widget.line);
      if (no == null) return;
